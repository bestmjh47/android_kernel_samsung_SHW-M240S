...
#if defined(CONFIG_MACH_COOPER_BASE_KT)  // M240K, M290K
#define USE_PER_INFO  
...
)
{
	...
	p += snprintf(p, MAX_WX_STRING, "GETPOWER = %d", pm);
	wrqu->data.length = p - extra + 1;
...
)
{
	...
	if (dwrq->length && extra) {
	    ...
	    if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
		...
		else if (strnicmp(extra, "GETPOWER", strlen("GETPOWER")) == 0)         //<-  GETPOWERMODE
			...
			ret = wl_iw_get_power_mode(dev, (union iwreq_data *)dwrq, extra);
#endif
...
const char *get_roam_status_string(int status)
{
	if ((0 <= status) && (status < 16))
		return STATUS_STRING[status];

	return STATUS_STRING[16];
}
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	iw = *(wl_iw_t **)netdev_priv(dev);
#ifdef ROAM_DEBUG
	if ((event_type != WLC_E_ESCAN_RESULT) && (event_type != WLC_E_TRACE) && (event_type != WLC_E_SCAN_COMPLETE) && (event_type != WLC_E_ROAM_START) && (event_type != WLC_E_ROAM_PREP))
#else
	...
	switch (event_type) {
			...
			case WLC_E_ROAM_PREP:
				WL_ERROR(("ROAM: Joining to %02X:%02X:%02X:%02X:%02X:%02X\n",
						e->addr.octet[0], e->addr.octet[1], e->addr.octet[2], e->addr.octet[3], e->addr.octet[4], e->addr.octet[5]));
				break;
		
			case WLC_E_ROAM_START:
				if (status == 0) {
					wlc_roam_t *roam = (wlc_roam_t *)data;
					int roamscan_new = roam->roamscan_new;
					WL_ERROR(("ROAM: Start %s scan, %d seconds, reason %u, %d channels\n",
						roamscan_new ? "new" :
						!(roam->cache_valid && roam->cache_numchannels > 0) ? "full" : "partial",
						roamscan_new || !(roam->cache_valid && roam->cache_numchannels > 0) ?
						roam->fullscan_period : roam->partialscan_period,
						ntoh32(e->auth_type), roam->cache_numchannels));
				} else if (status == 1) {
					int s = ntoh32(e->reason);
					WL_ERROR(("ROAM: Scan Complete Status %d, %s\n", s, get_roam_status_string(s)));
				} else if (status == 2) {
					roam_list_t *list = (roam_list_t *)data;
					int i;
					int n = ntoh32(e->auth_type);
					int last = ntoh32(e->reason);
					WL_ERROR(("ROAM: Scan Complete %d/%d\n", last, n));
					for (i=0; i<n; i++, list++) {
						printk("ROAM: [%02d] %02X:%02X:%02X:%02X:%02X:%02X %02X %d\n", i,
								list->BSSID[0], list->BSSID[1], list->BSSID[2],
								list->BSSID[3], list->BSSID[4], list->BSSID[5],
								list->chanspec, list->RSSI);
					}
				} else if (status == 3) {
					WL_ERROR(("ROAM: wlc_roam_scan() failed. err=%d\n", ntoh32(e->reason)));
				} else if (status == 4) {
					WL_ERROR(("ROAM: Roaming Scan Aborted with RSSI %d\n", ntoh32(e->reason)));
				}
				break;
#endif
	case WLC_E_TRACE:
